...
impl<'info> Initialize<'info> {
    ...
    /// The `escrow_bump` value is obtained in the instruction handler from `ctx.bumps`.
    pub fn initialize_escrow(
        ...
        // We use the seller's public key (not the admin) as the “initializer” in escrow.
    ...
    // (We intentionally remove any deposit() call so that no funds are transferred during escrow creation.)
}
